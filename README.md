...
- Live, interactive version: <https://americanmineralregistry.com/research/mineral-rights-by-state>
...
The dataset is built from national statutory surveys and primary sources. Dormant mineral law varies by state and changes over time, so the `note` field repeats a simple instruction, confirm against the current state code before relying on any single entry. This dataset is informational and is not legal advice.
